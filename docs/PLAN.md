...
| AC-15 | E-6(371px)·E-9(정확히 375px, 배포 URL) | — | 브라우저 없음, 미검증 | |

- 미검증으로 남은 AC-4·9·10·11·15는 작성자 브라우저 실행 증거(E-6·E-9)만 있다. **해소 담당·시점** `[결정]`: Director가 인증키를 `.env`에 넣은 뒤(T0) 실데이터 스냅샷을 배포하는 시점에, 브라우저를 가진 별도 검수 세션(sonnet, 배포 URL 대상)에 AC-4·9·10·11·15 재현을 발주한다. 그 결과로 §9.2 "검수자 독립 재현" 칸을 교체한다. 그때까지 "작성자 단독 확인"으로 표기한다.
